...
- 🔑 **Autentikált**: JWT Bearer token szükséges
- 👑 **Admin**: JWT Bearer token + admin jogosultság szükséges
...
- Token típus: Bearer token
- Token élettartam: 60 perc (alapértelmezett)
- Refresh token támogatás: Igen
...
```json
{
    ...
    "token": "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwOi8vbG9jYWxob3N0L2FwaS9yZWdpc3RlciIsImlhdCI6MTY0NTI0MzIwMCwiZXhwIjoxNjQ1MjQ2ODAwLCJuYmYiOjE2NDUyNDMyMDAsImp0aSI6IlJXZFFZdVZyMnVLdTdaeEciLCJzdWIiOiIxIiwicHJ2IjoiMjNiZDVjODk0OWY2MDBhZGIzOWU3MDFjNDAwODcyZGI3YTU5NzZmNyIsImlzX2FkbWluIjpmYWxzZX0.hashed_signature_here",
    "token_type": "Bearer"
}
```

> **🔐 JWT Token információk:**
> - Token formátum: `header.payload.signature` (Base64 URL encoded)
> - Élettartam: **60 perc** (3600 másodperc)
> - A payload tartalmazza: user ID (`sub`), admin státusz (`is_admin`), kibocsátási idő (`iat`), lejárat (`exp`)
> - A token automatikusan hitelesíti a felhasználót minden védett végponton
```
...
```json
{
    ...
    "token": "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwOi8vbG9jYWxob3N0L2FwaS9sb2dpbiIsImlhdCI6MTY0NTI0MzIwMCwiZXhwIjoxNjQ1MjQ2ODAwLCJuYmYiOjE2NDUyNDMyMDAsImp0aSI6IlJXZFFZdVZyMnVLdTdaeEciLCJzdWIiOiIxIiwicHJ2IjoiMjNiZDVjODk0OWY2MDBhZGIzOWU3MDFjNDAwODcyZGI3YTU5NzZmNyIsImlzX2FkbWluIjpmYWxzZX0.hashed_signature_here",
    "token_type": "Bearer"
}
```
...
**Jogosultság:** Autentikált (JWT Bearer token)
...
> **ℹ️ Kijelentkezés után:**
> - A JWT token érvénytelenné válik
> - Az ügyféloldali alkalmazásnak törölnie kell a tárolt tokent
> - Új bejelentkezés szükséges további műveletek végrehajtásához
```
...
> - A JWT token a `sub` claim-jében tárolja a user ID-t
> - Az `is_admin` claim jelzi az admin jogosultságot
> - Nincs szükség adatbázis lekérdezésre minden kérésnél (a token tartalmazza az infót)
```
...
**Jogosultság:** **ADMIN** (JWT Bearer token, admin szerepkör szükséges)
...
> **⚠️ Admin jogosultság:** A JWT token `is_admin` claim-je `true` kell legyen
...
2. Headers: 
   - `Content-Type: application/json`
   - `Authorization: Bearer {your_jwt_token}` (a login-nél kapott token)
3. Body → raw → JSON:
```json
{
    "user_id": 1,
...
2. Headers: 
   - `Authorization: Bearer {your_admin_jwt_token}` (az admin login-nél kapott token)
3. Send
...
3. Send
4. **Ellenőrizd:** 403 Forbidden hibát kapsz "Forbidden" üzenettel
...
**Package:** `php-open-source-saver/jwt-auth` (Laravel JWT Auth)

**JWT Token jellemzők:**
- **Formátum:** `header.payload.signature` (Base64 URL encoded)
- **Élettartam:** 60 perc (3600 másodperc)
- **Algoritmus:** HS256 (HMAC SHA-256)
- **Claims:**
  - `sub`: User ID
  - `is_admin`: Admin jogosultság (boolean)
  - `iat`: Kibocsátási időpont
  - `exp`: Lejárati időpont
  - `nbf`: Not Before időpont
...
### Autentikált végpontok (JWT Bearer token szükséges):
...
### Admin végpontok (JWT Bearer token + admin jogosultság szükséges):
**Felhasználók:**
...
- **Email:** admin@example.com
- **Jelszó:** admin123
- **Jogosultság ellenőrzés:** `is_admin` claim a JWT tokenben (boolean)
- **Ellenőrzés:** Minden admin controllerben if-fel: `if (!$request->user()->is_admin)`
