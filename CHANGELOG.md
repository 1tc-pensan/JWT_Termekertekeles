...
### 🔐 JWT Autentikáció Implementálva

**Major Changes:**
- ✅ JWT (JSON Web Token) autentikáció implementálva Laravel Sanctum helyett
- ✅ Package: `php-open-source-saver/jwt-auth` v2.8.3
- ✅ Token élettartam: 60 perc
- ✅ Admin jogosultság token payload-ban (`is_admin` claim)

**Features:**
- 🔑 JWT token generálás login és register során
- 🔒 Token alapú autentikáció middleware (`auth:api`)
- 👑 Admin jogosultság ellenőrzés controllerekben (if-fel)
- 📝 Teljes API dokumentáció JWT specifikus részletekkel
- ✨ Stateless autentikáció (nincs szerver oldali session)
...
**Updated Files:**
- `app/Http/Controllers/Api/AuthController.php` - JWT token kezelés
- `app/Models/User.php` - JWTSubject interface implementáció
- `config/auth.php` - JWT driver konfiguráció
- `config/jwt.php` - JWT beállítások (új fájl)
- `composer.json` - JWT package hozzáadva
- `API_DOKUMENTACIO.md` - Teljes JWT dokumentáció
- `README.md` - JWT leírás és setup lépések

**Breaking Changes:**
- ⚠️ Laravel Sanctum eltávolítva
- ⚠️ Token formátum megváltozott (Sanctum → JWT)
- ⚠️ Personal access tokens már nem támogatottak
- ⚠️ Kliens oldalon token tárolás módosítása szükséges

**Migration Guide:**
1. Futtasd: `composer install`
2. Futtasd: `php artisan jwt:secret`
3. Frissítsd a `.env` fájlt: `JWT_SECRET=...`
4. Kliens oldalon: használj JWT Bearer token-t Sanctum token helyett
